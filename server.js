...
app.listen(PORT, () => console.log(`Backend on ${PORT}`));  db.stock = db.stock.length ? db.stock : initialStock;
  db.fleet = db.fleet.length ? db.fleet : initialFleet;
  res.json(db);
});
...
app.delete('/client/:id', (req, res) => {
  const db = load();
  db.clients = db.clients.filter(c => c.id !== +req.params.id);
  db.cases = db.cases.filter(cs => cs.client_id !== +req.params.id);
  save(db);
  res.sendStatus(200);
});
...
app.post('/case', (req, res) => {
  ...
  db.nextIds.case = newCase.id;
  items.forEach(itm => {
    const s = db.stock.find(s => s.name.toLowerCase() === itm.toLowerCase());
    if (s && s.qty > 0) s.qty--;
  });
  save(db);
  res.json(newCase);
...
app.get('/export', (req, res) => {
  const db = load();
  const data = db.clients.map(c => ({ Name: c.name, ID: c.id_num, Phone: c.phone }));
...
app.listen(5001, () => console.log('Backend → http://localhost:5001'));
